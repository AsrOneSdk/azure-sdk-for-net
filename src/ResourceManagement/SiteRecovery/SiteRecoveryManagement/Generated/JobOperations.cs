...
namespace Microsoft.Azure.Management.SiteRecovery
{
    ...
    internal partial class JobOperations : IServiceOperations<SiteRecoveryManagementClient>, IJobOperations
    {
        ...
        public async Task<LongRunningOperationResponse> CancelAsync(string jobName, CustomRequestHeaders customRequestHeaders, CancellationToken cancellationToken)
        {
            ...
            cancellationToken.ThrowIfCancellationRequested();
            LongRunningOperationResponse result = await client.Jobs.GetCancelStatusAsync(response.Location, cancellationToken).ConfigureAwait(false);
                                ...
                                if (errorsArray != null && errorsArray.Type != JTokenType.Null)
                                {
                                    foreach (JToken errorsValue in ((JArray)errorsArray))
                                    {
                                        ...
                                        if (providerErrorDetailsValue != null && providerErrorDetailsValue.Type != JTokenType.Null)
                                        {
                                            ...
                                            {
                                                foreach (JProperty property in affectedObjectsSequenceElement)
                                                {
                                                    string affectedObjectsKey = ((string)property.Name);
                                                    string affectedObjectsValue = ((string)property.Value);
        ...
        /// A standard service response for long running operations.
        /// </returns>
        public async Task<LongRunningOperationResponse> GetCancelStatusAsync(string operationStatusLink, CancellationToken cancellationToken)
        {
            // Validate
        ...
        public async Task<JobListResponse> ListAsync(JobQueryParameter parameters, CustomRequestHeaders customRequestHeaders, CancellationToken cancellationToken)
        {
            ...
            url = url + "/replicationJobs";
            List<string> queryParameters = new List<string>();
            if (parameters != null && parameters.ToJson() != null)
            {
                queryParameters.Add("filter=" + Uri.EscapeDataString(parameters.ToJson().ToString()));
            }
            queryParameters.Add("api-version=2015-11-10");
            if (queryParameters.Count > 0)
            {
